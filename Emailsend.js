...
const Emailsend = ({navigation,route}) => {
  ...
  const ReSendOtp = async (e) => {
    e.preventDefault();
    await axios.post("http://192.168.128.218:3003/users", user1).catch((err)=>{console.log(err)})

  };

  useEffect(() => {
    axios.post("http://192.168.128.218:3003/users", user1).catch((err)=>{console.log(err)})
  }, [])

  function Verify(){
    if (otp === user1.description) {
      axios.post("http://192.168.128.218:3003/addUser1",{username,data1,Password,ConfirmPassword}).then((res)=>{
